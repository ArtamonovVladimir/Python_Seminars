...
def winner_check(user, bots):
    global WINNER, BOT_COUNT, PLAYER_COUNT
...
def counts (update, context):
    context.bot.send_message(update.effective_chat.id, f"У бота: {BOT_COUNT} побед(а).")
    context.bot.send_message(update.effective_chat.id, f"У пользователя: {PLAYER_COUNT} побед(а).")
...
counts_handler = CommandHandler('counts', counts)
stop_handler = CommandHandler('stop', stop)


dispatcher.add_handler(start_handler)
dispatcher.add_handler(still_handler)
dispatcher.add_handler(counts_handler)
